...
plot_map <- function(path_to_csv, path_to_store, width, height) {
  ...
  fig <- plot_geo(df, lat = ~Latitud, lon = ~Longitud, width = width, height = height)
  fig <- fig %>% add_markers(
    text = ~paste(paste("Magnitud", Magnitud), paste("Profundidad", Profundidad), sep = "<br />"),
    color = ~Magnitud, symbol = I("circle"), size = ~Profundidad, hoverinfo = "text"
  )
  fig <- fig %>% colorbar(title = "Magnitud de terremotos<br />")
  fig <- fig %>% layout(
    title = 'Mapa de terremotos', geo = g,
